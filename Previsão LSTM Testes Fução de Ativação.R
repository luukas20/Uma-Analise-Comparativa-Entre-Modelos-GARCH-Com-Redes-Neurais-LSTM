...
# Modelo Sigmoide Adam ----------------------------------------------------

modelo1 <- keras::keras_model_sequential() |>
  # arquitetura da rede
  keras::layer_lstm(units = 128, activation = 'sigmoid', return_sequences = T,input_shape = c(15,1)) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.06) |>
  keras::layer_lstm(units = 64, activation = 'sigmoid', return_sequences = T) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.05) |>
  keras::layer_lstm(units = 32, activation = 'sigmoid') |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.04) |>
...
modelo1 <- keras::keras_model_sequential() |>
  # arquitetura da rede
  keras::layer_lstm(units = 128, activation = 'tanh', return_sequences = T,input_shape = c(15,1)) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.06) |>
  keras::layer_lstm(units = 64, activation = 'tanh', return_sequences = T) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.05) |>
  keras::layer_lstm(units = 32, activation = 'tanh') |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.04) |>
...
modelo1 <- keras::keras_model_sequential() |>
  # arquitetura da rede
  keras::layer_lstm(units = 128, activation = keras::layer_activation_relu(), return_sequences = T,input_shape = c(15,1)) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.06) |>
  keras::layer_lstm(units = 64, activation = keras::layer_activation_relu(), return_sequences = T) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.05) |>
  keras::layer_lstm(units = 32, activation = keras::layer_activation_relu()) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.04) |>
  keras::layer_dense(units = 1, activation = keras::layer_activation_relu()) |>
  # backpropagation
  keras::compile(
    loss = 'mean_squared_error',
    optimizer = keras::optimizer_adam()
...
# Modelo Leaky Relu Adam --------------------------------------------------

modelo1 <- keras::keras_model_sequential() |>
  # arquitetura da rede
  keras::layer_lstm(units = 128, activation = keras::layer_activation_leaky_relu(), return_sequences = T,input_shape = c(15,1)) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.06) |>
  keras::layer_lstm(units = 64, activation = keras::layer_activation_leaky_relu(), return_sequences = T) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.05) |>
  keras::layer_lstm(units = 32, activation = keras::layer_activation_leaky_relu()) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.04) |>
  keras::layer_dense(units = 1, activation = keras::layer_activation_leaky_relu()) |>
  # backpropagation
  keras::compile(
    loss = 'mean_squared_error',
    optimizer = keras::optimizer_adam()
...
modelo1 <- keras::keras_model_sequential() |>
  ...
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.05) |>
  keras::layer_lstm(units = 32, activation = keras::layer_activation_leaky_relu(alpha = 0.25)) |>
  #keras::layer_batch_normalization() |>
  keras::layer_dropout(0.04) |>
  keras::layer_dense(units = 1, activation = keras::layer_activation_leaky_relu(alpha = 0.25)) |>
  # backpropagation
  keras::compile(
    loss = 'mean_squared_error',
    optimizer = keras::optimizer_adam()
...
while(i <= n_future){
  if(length(list_output_steps) > 15){
    input_steps = list_output_steps[,-1] |> matrix(ncol = 15,nrow = 1)
    pred = predict(modelo1,input_steps)
    print(paste('Dia',as.Date('2024-01-31')+i,'Valores de saida -> ', pred * amp + me))
    list_output_steps <- cbind(input_steps,pred)
    pred10[i] <- pred
    i=i+1
  }
