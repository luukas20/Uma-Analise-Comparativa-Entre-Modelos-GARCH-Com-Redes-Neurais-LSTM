...
modelo1 <- keras::keras_model_sequential() |>
  ...
  keras::layer_lstm(units = 32, activation = keras::layer_activation_leaky_relu(alpha = 0.3)) |>
  #keras::layer_batch_normalization() |>
...
ajuste1 <- modelo1 |> keras::fit(
  ...
  epochs = 100,
  batch_size = 16,
  #callbacks = list(callback_early_stopping(monitor='val_loss', mode='min', patience=50, min_delta=0.00024, baseline=0.00024, restore_best_weights = TRUE)),
...
ggplot(ajt) +
  ...
  geom_line(aes(x = 1:100, y = val_loss, color = "Validação"), size = 1) +
  geom_point(aes(x = 1:100, y = val_loss, color = "Validação"), size = 2) +
  scale_color_manual(values = c("Treinamento" = "goldenrod2", "Validação" = "cyan3")) +
  labs(x = "Época", y = "MSE") +
  #facet_zoom(xlim = c(as.Date("2024-02-01"), as.Date("2024-02-29"))) +
  theme_minimal() + theme(legend.position = "bottom", legend.title = element_blank())

dev.copy2pdf(file="C:\\Users\\lucas\\OneDrive - 5qgclf\\Documentos\\UFS\\10 Periodo\\TCC\\TCC Latex\\figuras\\Treino e Teste LSTM.pdf", width = 7, height = 5)
